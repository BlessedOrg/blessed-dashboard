...
export const TicketsDashboardContent = ({ currentTabIndex, onTabChange, eventId, appId }: AppDashboardContentProps) => {
  const {
    ...
    mutate,
  } = useSWR<ITicket[]>(`${apiUrl}/private/tickets/${appId}/${eventId}`, fetcherWithToken);
  ...
  const contentPerTab: Record<number, JSX.Element> = {
    ...
    2: <AdminDashboard tickets={ticketsData || []} isTicketsView={true} />,
  ...
  return ticketsLoading ? <LoadingDashboardSkeleton /> : contentPerTab[currentTabIndex] || null;
};
